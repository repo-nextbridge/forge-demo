...
EOF
    ...
    totem_door="$(origin_for "$FORGE_TAILNET_HOST" "$tsch" "$tprt" "${FORGE_TOTEM_HTTP_PORT:-8203}")"
    ...
    if bench_bind_is_loopback; then
      ...
      while read -r label door; do
        [ -n "${door:-}" ] || continue
        case "$door" in "http://$FORGE_TAILNET_HOST"|"http://$FORGE_TAILNET_HOST:"*) unreachable="$unreachable$label $door
" ;; esac
      done <<EOF
vitrine $origin
totem $totem_door
