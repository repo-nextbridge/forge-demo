...
async function sealTheCoffees({ command, log }, known) {
  ...
  const blind = [];
  for (const handle of data.publish_also.handles) {
    // A48 folded the second mark into this same pass, so `additions` is the WHOLE of what the counter adds
    // to this coffee — an empty bag means there is nothing to write and no command to spend.
