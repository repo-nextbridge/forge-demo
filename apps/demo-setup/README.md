...
It exists twice over: this box wants its shops to wear their own names in their **shop window**, **and** it
is the shortest true answer to *"how does a customer extend Forge?"* — three blocks, one shared render, no
admin page, no scope, no table, no migration.
...
⛔ **There was a fourth until pk28 — the login box (`storefront:account.brand`) — and it left on the axis of
...
## Three components, one drawing

`block/marks.tsx` holds one implementation (`Mark`) and three exports. Sharing the render is why *"one logo
becoming another"* is a config change and not three maintenance jobs; declaring one per place is what lets a
store put its mark in each of them **at all** — `placement: 'single'` is enforced by the kernel per *(store,
app, component)*, so one component in a second slot is refused with `conflict`.

The three differ only where the place differs, which today is one thing: the footer's column is the one that
...
- **Nothing configured ⇒ nothing drawn.** These slots *replace* the front's own wordmark, so a fallback
