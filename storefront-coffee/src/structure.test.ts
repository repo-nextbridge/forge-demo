...
// CHECKOUT-APP (K1) moved them into `@forgeco/storefront-kit` with the rest of the chrome, and the scan
...
test('★ the vitrine imports nothing from the checkout (a guard test excepted, and only that)', () => {
  ...
  expect(
    ...
    `the vitrine reaches into apps/checkout:\n  ${offenders.join('\n  ')}\n\n` +
      "Shared code goes to @forgeco/storefront-kit; the sibling's slot registry arrives as data " +
